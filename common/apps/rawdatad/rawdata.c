...
static void write_productinfo(int partition, int block)
{
    FILE *fp;
...
static void write_adc(char* buffer )
{
    FILE *fp;
...
int write_adc_data(char *buffer, int size)
{
    int fd = -1;
    int ret = 0;
...
int	read_adc_data(char *buffer,int size)
{
    ...
    if(0x00000001 != ((unsigned long*)&cali_info)[13]){
        DBG("%s: not cali ! \n", __FUNCTION__);
        close(fd);
        return 1;
    }
...
static void init_adc(void)
{
    ...
    if(-1 == ret){
        DBG("%s: read_adc_data error !\n",__FUNCTION__);
        return;
    }

    if(1 == ret && adc_cal){
        write_adc_data(adc_value,size);
    }else if(0 == ret && !adc_cal){
        write_adc(adc_buffer);
    }else{
        DBG("%s: do nothing !\n",__FUNCTION__);
    }
}
