...
SprdOverlayPlane::SprdOverlayPlane(FrameBufferInfo *fbInfo, SprdBufferManager *BufferManager)
...
struct bufferInfo *SprdOverlayPlane::dequeueBuffer()
{
    bool ret = false;
...
void SprdOverlayPlane::UpdateOverlayLayer(SprdHWLayer *l)
{
...
bool SprdOverlayPlane::flush()
{
    ...
    if (mDumpFlag)
    {
        dumpOverlayImage(BaseContext, NULL, CurrentOverlayVirAddr, mDumpFlag);
    }
...
struct bufferInfo *SprdOverlayPlane::getPlaneBuffer()
{
    if (mBuffer == NULL)
    {
        //mBuffer = SprdDisplayPlane::dequeueBuffer();
