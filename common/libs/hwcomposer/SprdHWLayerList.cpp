...
void SprdHWLayerList::dump_layer(hwc_layer_t const* l) {
...
bool SprdHWLayerList::IsHWCLayer(hwc_layer_t *AndroidLayer)
{
...
bool SprdHWLayerList:: updateGeometry(hwc_layer_list_t *list)
{
    mLayerCount = 0;
    mRGBLayerCount = 0;
    mYUVLayerCount = 0;
...
bool SprdHWLayerList:: revistGeometry(bool *forceOverlayFlag)
{
    ...
    {
        hwc_layer_t *layer = mLayerList[i].getAndroidLayer();

        SkipLayerFlag = !(IsHWCLayer(layer));
        if (SkipLayerFlag)
        {
            FBLayerCount++;
            continue;
...
void SprdHWLayerList:: ClearFrameBuffer(hwc_layer_t *l, unsigned int index)
{
    if (index != 0)
    {
...
{
    hwc_layer_t *layer = l->getAndroidLayer();
...
void SprdHWLayerList:: resetOverlayFlag(SprdHWLayer *l)
{
    ...
    l->setPlaneType(PLANE_INVALIDATE);
}
...
bool SprdHWLayerList::checkHWLayerList(hwc_layer_list_t* list)
{
    if (list == NULL)
    {
