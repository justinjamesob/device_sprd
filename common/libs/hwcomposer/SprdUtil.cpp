...
void OSDTransform::onStart(SprdHWLayer *l, struct bufferInfo *buffer)
{
                         ...
                         struct bufferInfo *buffer1, struct bufferInfo *buffer2)
{
    ...
    if (l1 && buffer1)
    {
        ...
        if (tmpBuffer == NULL)
        {
            tmpBuffer = static_cast<struct bufferInfo *>(mBM->alloc(mFBInfo->fb_width, mFBInfo->fb_height, format));
            if (tmpBuffer == NULL)
            {
        ...
        hwc_layer_t *layer = l1->getAndroidLayer();
        ruct sprdRect *srcRect = l1->getSprdSRCRect();
        ...
        int ret = transform_layer(private_h->phyaddr, private_h->base, private_h->format,
                      ...
                      buffer1->phyAddr, buffer1->virAddr, dstFormat,
                      FBRect->w, FBRect->h, srcRect,
                      tmpBuffer->phyAddr, tmpBuffer->virAddr);
        if (ret != 0)
        {
...
int SprdUtil::composerLayers(SprdHWLayer *l1, SprdHWLayer *l2, struct bufferInfo *buffer1, struct bufferInfo *buffer2)
{
    int32_t ret = 0;
    static int openFlag = 0;
    hwc_layer_t *layer1 = NULL;
    hwc_layer_t *layer2 = NULL;
    int layer2_Format = -1;
    ...
    if (video_check_result || osd_check_result) {
        ...
        //config output
        current_overlay_vaddr = (unsigned int)buffer->virAddr;
                    ...
                    || ((gsp_cfg_info.layer0_info.clip_rect.rect_h * 4) < gsp_cfg_info.layer0_info.des_rect.rect_w))))
        {
            ...
            ALOGI_IF(mDebugFlag,"GSP process layers L%d,Ld [p%d], the output buffer phyAddr:%p, virAddr:%p",__LINE__,gsp_cfg_info.layer_des_info.pitch, (void *)buffer->phyAddr, (void *)buffer->virAddr);
             ret = mGspDev->GSP_Proccess(&gsp_cfg_info);
         }
#else
        ALOGI_IF(mDebugFlag,"GSP process layers L%d,Ld [p%d], the output buffer phyAddr:%p, virAddr:%p",__LINE__,gsp_cfg_info.layer_des_info.pitch, (void *)buffer->phyAddr, (void *)buffer->virAddr);
                           ...
                           gpu_transform_info_t *transformInfo)
{
    ...
    if(l1 && buffer1)
    {
        ...
        transformInfo->video.dstVirt = (uint32_t)buffer1->virAddr;
    ...
    if(l2 && buffer2)
    {
        hwc_layer_t *layer = l2->getAndroidLayer();
        ...
        transformInfo->osd.dstVirt = (uint32_t)buffer2->virAddr;
